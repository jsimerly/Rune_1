...
class MapLayout:
    ...
    def generate_map(self, surface) -> Dict[Tuple[int, int], GameTile]:
        hexes = self.shape(**self.shape_params)

        game_map = {}
        for hex in hexes:
            game_map[(hex.q, hex.r)] = Grass(hex.q, hex.r, self.layout, surface)

        for tile_class, coords in self.special_tiles.items():
            for cord in coords:
                q, r = cord
                game_map[(q,r)] = tile_class(q, r, self.layout, surface)
